...
public class ImdbServiceTestSuite {
    ...
    public void shouldReturnNewlyScrapedImdbReview() throws IOException {
        ...
        when(imdbReviewRepository.findByImdbId("testImdbId"))
                .thenReturn(Collections.singletonList(reviewNew));
        when(imdbReviewMapper.mapToImdbReviewDtoList(Collections.singletonList(reviewNew)))
                .thenReturn(Collections.singletonList(reviewDtoNew));
        ...
        verify(imdbReviewRepository, times(1)).saveAll(ArgumentMatchers.anyCollection());
